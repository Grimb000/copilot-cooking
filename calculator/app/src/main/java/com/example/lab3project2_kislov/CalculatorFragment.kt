...
class CalculatorFragment : Fragment(R.layout.fragment_calculator) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            restoreCalcState(savedInstanceState)
        }
    }
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putString(KEY_CALC_INPUT, calcCurrentInput)
        outState.putString(KEY_CALC_PENDING, calcPendingOperator?.name)
        outState.putBoolean(KEY_CALC_JUST_EVALUATED, calcJustEvaluated)
        if (calcStoredValue != null) {
            outState.putDouble(KEY_CALC_STORED, calcStoredValue!!)
        }
        outState.putString(KEY_CALC_DISPLAY, tvCalcDisplay.text.toString())
        outState.putString(KEY_CALC_ERROR, tvCalcError.text.toString())
    }
    ...
    private fun restoreCalcState(savedInstanceState: Bundle) {
        calcStoredValue = if (savedInstanceState.containsKey(KEY_CALC_STORED)) {
            savedInstanceState.getDouble(KEY_CALC_STORED)
        } else {
            null
        }
        calcPendingOperator = savedInstanceState.getString(KEY_CALC_PENDING)
            ?.let { CalcOperator.valueOf(it) }
        calcCurrentInput = savedInstanceState.getString(KEY_CALC_INPUT, "")
        calcJustEvaluated = savedInstanceState.getBoolean(KEY_CALC_JUST_EVALUATED, false)
        tvCalcError.text = savedInstanceState.getString(KEY_CALC_ERROR, "")
        updateCalcDisplay(
            savedInstanceState.getString(
                KEY_CALC_DISPLAY,
                getString(R.string.calc_display_default)
            ) ?: getString(R.string.calc_display_default)
        )
    }
    ...
    private companion object {
        private const val KEY_CALC_INPUT = "calc_input"
        private const val KEY_CALC_PENDING = "calc_pending"
        private const val KEY_CALC_JUST_EVALUATED = "calc_just_evaluated"
        private const val KEY_CALC_STORED = "calc_stored"
        private const val KEY_CALC_DISPLAY = "calc_display"
        private const val KEY_CALC_ERROR = "calc_error"
    }
}
